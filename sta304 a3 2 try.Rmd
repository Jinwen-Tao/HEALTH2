...
Data_health<- read.csv("/Users/dilireba/Desktop/AA2vfFTU.csv")
...
# This is a second version of our data 
# We select variables dvn2a, h2hrs,d3, dvyrsmod,n3 and dvsex 
# from the raw data and rename them with Health_Satisfaction_level,
# Hours_of_Sleep_per_night, Flu_shot_yes, Smoking_Years,
# Degree_of_Relaxation,Gender respectively. 
...
Health_Satisfication_level2 <- as.numeric(Effects_on_health$Health_Satisfication_level)
...
Effects_on_health_lm <-lm(Health_Satisfication_level ~ Hours_of_Sleep_per_night + Smoking_Years + Flu_shot_yes + Stress_Levels, data = Effects_on_health)

summary(Effects_on_health_lm)
...
#into the data frame by creating a new column "percentage_yes" in percentage representation.
...
# The pie chart shows the distribution of Health Satisfaction level with
# people taking flu shots
...
# The combing pie chart that show the Health Satisfaction 
# level with and without taking flu shot.
figure <- ggarrange(
...
# The bar chart of degree of relaxation affecting the 
# respondents' health satisfacton levels.
...
```{r}
# The bar chart of number of people who are group by years of smoking in different health satisfaction levels.
Smoking_h <- Effects_on_health %>% 
  mutate(Smoking_Years_Groups = case_when(Smoking_Years == 0 ~ "0",
                                          Smoking_Years > 0  & Smoking_Years <= 10 ~ '1 - 10',
                                          ...
                                          Smoking_Years  > 60 ~ '61 - more '
                                            )) %>%
  select(Smoking_Years_Groups, Health_Satisfaction_level)

 Smoking_VS_Health <- ggplot(Smoking_h, 
                                      aes(x = Smoking_Years_Groups,
                                        fill = Health_Satisfaction_level)) + 
   geom_bar() + theme_classic() +
  labs(title = "How years as a smoker affects individuals' health satisfacton level",
       x = "Groups of smoking years",
       y = "Number of people")  + 
